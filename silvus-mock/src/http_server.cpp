...
void HttpServer::serve() {
    ...
    try {
        ...
        std::cerr << "[silvus-mock] REST server failed: " << e.what() << std::endl;
        std::cerr << "[silvus-mock] Hint: Port " << port_ << " may be already in use."
                  << " Check for another silvus-mock instance or set SILVUS_MOCK_HTTP_PORT." << std::endl;
        throw; // Re-throw so main() can exit with proper code
    } catch (const std::exception& e) {
        std::cerr << "[silvus-mock] REST server fatal error: " << e.what() << std::endl;
        throw;
    }
}
